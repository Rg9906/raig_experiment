...
% Sequential light-to-dark blue colormap for Fig. 5 (the misspecification
% heatmap). pgfplots has no built-in "Blues" colormap (that is a
% matplotlib/ColorBrewer name); this is a hand-defined equivalent.
\pgfplotsset{
  colormap={raigblues}{rgb255=(205,226,251) rgb255=(13,54,107)}
}
...
\centering
\footnotesize
\begin{tabular}{@{}lp{0.62\columnwidth}c@{}}
\toprule
...
\begin{axis}[
  ...
  grid=both, grid style={line width=0.2pt, draw=black!15},
  colormap name=raigblues, colorbar, point meta min=0, point meta max=0.80,
...
true crossover, $R=500$ trials/cell, $T=20$) as a function of assumed against
true crossover. The diagonal is correct specification. Accuracy is dominated
by the true crossover (rows), falling from 49--78\% at $\varepsilon=0.02$ to
$\le 1.4\%$ once $\varepsilon\ge0.18$. At $5\times$ the original resolution
($R=500$/cell, re-run from the original $R=100$/cell to check this was not
an under-powered artefact) the within-row diagonal effect remains small and
does not resolve into a clean pattern: the correctly-specified cell is the
row maximum in only 1 of 5 rows. This is consistent with
Section~\ref{sec:tiers}'s uniform-specification argument -- under
\emph{homogeneous} true noise, a constant $\hat\varepsilon$ shifts every
question's RAIG score by the same capacity term regardless of its exact
value, so misspecification here degrades the Bayesian update's confidence
without changing which questions get \emph{selected}; only the row (the true
noise level) has a first-order effect on accuracy.}
\label{fig:misspec}
\end{figure}
%----------------------------------------------------------------------
...
heterogeneous $\lambda=1.0$, $R=150$ trials, confidence threshold 0.90.
ms/turn is the \emph{median} of 5 independent runs on this shared machine
(individual runs ranged 57--516ms owing to unrelated background load; see
\texttt{results/latency\_reconciled.json} for the full breakdown). Treat the
absolute figure as order-of-magnitude, not a precise benchmark -- the robust
finding is that both selectors cost the same per-turn compute, since both
reduce to one matrix--vector product plus a vectorised entropy transform. At
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{ms/turn (median of 5)} & \textbf{Q to threshold} & \textbf{Top-5 (\%)} \\
\midrule
IG+soft-uniform & 64.2 & 20.0 (0.7\% reached) & 10.7 \\
RAIG-tiered & 66.3 & 19.9 (2.7\% reached) & 22.0 \\
\bottomrule
\end{tabular}
\end{table}
